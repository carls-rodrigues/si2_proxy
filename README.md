...
* **Falha de segurança identificada (XSS Refletido):** Durante uma revisão de qualidade de código, a IA apontou uma vulnerabilidade de **Cross-Site Scripting (XSS) refletido** nas páginas de erro do proxy. As mensagens de erro (ex.: "URL inválida") interpolavam diretamente no HTML a URL fornecida pelo usuário, sem qualquer tratamento. Isso permitia que uma URL maliciosamente construída (contendo `<script>` ou outras *tags*) fosse refletida e executada no navegador da vítima. A correção, também orientada pela IA, consistiu em criar uma função `escape_html` que converte os caracteres especiais (`& < > " '`) em suas entidades HTML antes de inseri-los na resposta, neutralizando a injeção. O mesmo tratamento foi aplicado ao nome do domínio exibido na página de bloqueio.
